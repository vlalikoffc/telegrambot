...
from system.messages import RATE_LIMITER, send_status_reply_message
from system.state import format_chat_label, ensure_chat_state
...
class OwnerInfoManager:
    ...
    async def _schedule_delete(
        ...
        try:
            ...
            logging.warning(
                "Chat %s: failed to delete owner info message: %s",
                format_chat_label(chat_id, ensure_chat_state(app.bot_data["state"], chat_id)),
                exc,
            )
        except Exception as exc:  # pragma: no cover - defensive
            logging.warning(
                "Chat %s: unexpected delete error: %s",
                format_chat_label(chat_id, ensure_chat_state(app.bot_data["state"], chat_id)),
                exc,
            )
        finally:
